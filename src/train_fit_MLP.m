function data = train_fit_MLP(data, SNR, pythonpath)
...
fsphere = @(p,x) exp(-my_murdaycotts(delta,smalldel,p,2,x) );
fball = @(p,x) exp(-p.*x);
...
p6 = (1-0.5).*rand(Nset,1)+0.5;      % S0
p7 = (4e3-10).*rand(Nset,1)+10;      % T1
p8 = (3-0.5).*rand(Nset,1)+0.5;      % Dees

f = @(p,B) p(6).*(1-exp(-TR./p(7))).*( (1-p(1)).*exp(-TE./p(4)).*fstick(8,B) + p(1).*(p(2).*exp(-TE./p(5)).*fsphere(p(3),B) + (1-p(2)).*exp(-TE./p(4)).*fball(p(8), B)) );
p_fit = [p1 p2 p3 p4 p5 p6 p7 p8];


% Build the training set
database = zeros(size(p_fit,1), length(B));
